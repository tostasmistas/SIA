...
Mais uma vez utilizou-se um \textit{guard ring} cuja função é proteger e polarizar o circuito tal como foi dito nos blocos anteriores. É feita então uma ligação entre este o \textit{guard ring} do bloco \#1 ficando-se assim com a polarização feita a \texttt{VDD} tal como desejado.
...
Para realizar estas ligações entre blocos foi necessário ter especial atenção facto de que o material dos \textit{guard rings} é \texttt{Metal1} pelo que estas teriam que necessariamente ser feitas usando \texttt{Metal2} para evitar curto circuitos. O aspecto final do layout é assim o da imagem seguinte.
...
Feitas estas ligações, de forma a proteger ainda mais o circuito colocou-se um \textit{guard ring}, polarizado a \texttt{Ground}, em torno da forma final tal como se observa acima. Com esta polarização pode então fazer-se uma ligação por \texttt{Metal1} entre este e o \textit{guard ring} do bloco \#4 ficando assim também polarizado. Este \textit{guard ring} no entanto não é rectangular, tendo sido ajustado à forma do circuito final poupando-se ao máximo a área consumida. Neste \textit{guard ring} foram também inseridos todos os \textit{pins} de forma a que nenhum ficasse solto no circuito. Introduziram-se assim os \textit{pins} \texttt{In}, \texttt{Out}, \texttt{Vin+}, \texttt{Vin-}, \texttt{Ibias} e \texttt{Ibias 2}. Como todos foram inseridos no \textit{guard ring} exterior são do material \texttt{Metal2} \texttt{Pin} sendo a sua \textit{label} do material complementar, ou seja \texttt{Pin} \texttt{Metal2}, tal como já mencionado anteriormente. Da mesma forma coloca-se um \textit{pin} para fazer a ligação a \texttt{Grround} cujo o material é \texttt{Metal1} \texttt{Pin} e a \textit{label} é \texttt{Pin} \texttt{Metal1}.
...
Também na simulação por \textit{corners} os únicos resultados negativos foram na margem de fase, sendo que no entanto a situação melhora face ao caso após  aplicação de multiplicidade e \textit{fingers}.
...
\begin{figure}[H]
	\centering
	\includegraphics[keepaspectratio=true, scale=0.50]{teoricas/area}
	\vspace{-0.5em}
	\caption{Dimensões para cálculo da área.}
	\vspace{-0.8em} 
\end{figure}
...
Com este \textit{final target} obteve-se assim experiência relativamente às boas práticas e à forma como se deve proceder na realização de um layout, sendo que este relatório será, sem dúvida, uma referência para \textit{layouts} futuros realizados pelos membros deste grupo, não só para relembrar nas coisas certas a fazer, mas também as que não se devem evitar.
