...
Os $V_{DB_2}$ e $V_{DB_9}$ é dado pelas seguintes equações:
	\vspace{-3mm}
	\begin{equation}
	V_{DB_2} = V_D - V_B = V_{DD} - R_{o_2} I_D - V_{DD} = - R_{o_{11\_2}} I_D ;
	\end{equation}
	\vspace{-3mm}
	\begin{equation}
	V_{DB_9} = V_D - V_B = V_{DD} - R_{o_9} I_D - GND = V_{DD} - R_{o_9} I_D ;
	\end{equation}
	
Analisando o circuito observa-se que a resistência $R_{o_2}$ depende da resistência de saída do transístor $M_{11\_2}$  
\improvement{ainda haverá mais texto aqui?}
...
\improvement{referir que se mudam aos pares}
